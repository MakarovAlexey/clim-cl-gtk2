...
#+(or)
(defparameter *key-table* (clim-cl-gtk2::collect-key-table))
#+(or)
(print-key-table)
...
(defun cwd ()
  (slot-value (asdf:find-system :clim-cl-gtk2) 'asdf::relative-pathname))
	...
	do
	  (when value
		...
		(when (and (eql (second clause) name)
		  ...
		  (return))))))
    (with-open-file (s (merge-pathnames "keys.lisp" (cwd))
		     :direction :output
		     :if-exists :rename-and-delete)
      (write-line ";; autogenerated by keygen.lisp" s)
      (print '(in-package :clim-cl-gtk2) s)
      (loop
	  for (value . spec)
	      :in (sort (loop
