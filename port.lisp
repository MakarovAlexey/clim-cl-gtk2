...
(defclass cl-gtk2-pointer (standard-pointer)
  ((port :initarg :port :accessor port)
   (cursor :accessor pointer-cursor :initform :upper-left)))

(defclass cl-gtk2-port (basic-port)
...
;;;(defmethod print-object ((object cl-gtk2-port) stream)
;;;  (print-unreadable-object (object stream :identity t :type t)
;;;    (format stream "~S ~S" :id (slot-value object 'id))))

(defun parse-cl-gtk2-server-path (path)
  path)
...
;;; package instead.
(setf (get :cl-gtk2 :port-type) 'cl-gtk2-port)
(setf (get :cl-gtk2 :server-path-parser) 'parse-cl-gtk2-server-path)

(defmethod initialize-instance :after ((port cl-gtk2-port) &rest initargs)
  (declare (ignore initargs))
  (setf (events-head port) (list nil))
  (setf (events-tail port) (events-head port))
  (setf (port-pointer port) (make-instance 'cl-gtk2-pointer :port port))
  (push (make-graft port) (climi::port-grafts port))
  ;; FIXME: it seems bizarre for this to be necessary
  (push (make-instance 'cl-gtk2-frame-manager :port port)
...
(defmethod destroy-port :before ((port cl-gtk2-port))
  )
...
;;; do this for us then?
(defmethod port-allocate-pixmap ((port cl-gtk2-port) sheet width height)
  (let ((pixmap (make-instance 'cl-gtk2-pixmap
		  :sheet sheet
		  :width width
		  :height height
		  :port port)))
...
(defmethod port-deallocate-pixmap ((port cl-gtk2-port) pixmap)
...
(defmethod realize-mirror
    ((port cl-gtk2-port) (sheet climi::top-level-sheet-pane))
  (let ((mirror (realize-window-mirror port sheet GTK_WINDOW_TOPLEVEL)))
    (port-enable-sheet port sheet)
    mirror))

(defmethod realize-mirror
    ((port cl-gtk2-port) (sheet climi::unmanaged-top-level-sheet-pane))
...
(defmethod realize-mirror ((port cl-gtk2-port) (sheet mirrored-sheet-mixin))
  (with-gtk ()
...
(defmethod realize-mirror ((port cl-gtk2-port) (sheet native-widget-mixin))
  (with-gtk ()
...
(defmethod realize-mirror :after ((port cl-gtk2-port) (sheet gtk-menu-bar))
  (dolist (menu (gtk-menu-bar-contents sheet))
    (unless (integerp menu)		;?
      (sheet-adopt-child sheet menu))))

(defmethod realize-mirror ((port cl-gtk2-port) (sheet gtk-menu))
...
(defmethod realize-mirror ((port cl-gtk2-port) (sheet gtk-nonmenu))
  (unless (climi::port-lookup-mirror port sheet)
    (with-gtk ()
      (let* ((menu-item
...
(defmethod realize-mirror ((port cl-gtk2-port) (pixmap-sheet climi::pixmap))
...
(defmethod destroy-mirror ((port cl-gtk2-port) (sheet mirrored-sheet-mixin))
  (with-gtk ()
    (let ((mirror (climi::port-lookup-mirror port sheet)))
      (destroy-mediums mirror)
...
(defmethod destroy-mirror :after
	   ((port cl-gtk2-port) (sheet native-widget-mixin))
  (setf (native-widget sheet) nil))

(defmethod destroy-mirror ((port cl-gtk2-port) (pixmap-sheet climi::pixmap))
  (with-gtk ()
...
(defmethod destroy-mirror ((port cl-gtk2-port) (pixmap-sheet gtk-menu))
  (with-gtk ()
    (let ((mirror (climi::port-lookup-mirror port pixmap-sheet)))
      (when mirror
	(climi::port-unregister-mirror port pixmap-sheet mirror)))))

(defmethod destroy-mirror ((port cl-gtk2-port) (pixmap-sheet gtk-nonmenu))
  (with-gtk ()
    (let ((mirror (climi::port-lookup-mirror port pixmap-sheet)))
      (when mirror
	(climi::port-unregister-mirror port pixmap-sheet mirror)))))
...
(defmethod port-set-mirror-region
    ((port cl-gtk2-port) (mirror window-mirror) mirror-region)
  (with-gtk ()
...
(defmethod port-set-mirror-region
    ((port cl-gtk2-port) (mirror mirror) mirror-region)
...
(defmethod port-set-mirror-region
    ((port cl-gtk2-port) (mirror native-widget-mirror) mirror-region)
  (with-gtk ()
...
(defmethod port-set-mirror-transformation
    ((port cl-gtk2-port) (mirror window-mirror) mirror-transformation)
  (with-gtk ()
    (multiple-value-bind (x y)
...
(defmethod port-set-mirror-transformation
    ((port cl-gtk2-port) (mirror mirror) mirror-transformation)
  (with-gtk ()
...
(defmethod port-set-mirror-transformation
    ((port cl-gtk2-port) (mirror native-widget-mirror) mirror-transformation)
  (with-gtk ()
...
(defmethod port-enable-sheet
    ((port cl-gtk2-port) (sheet mirrored-sheet-mixin))
  (with-gtk ()
    (let ((mirror (climi::port-lookup-mirror port sheet)))
...
(defmethod mirror-transformation ((port cl-gtk2-port) mirror)
  ())

;; die sind verkehrt, die gibt's gar nicht
;;;(defmethod port-set-sheet-transformation
;;;    ((port cl-gtk2-port) (graft graft) transformation)
;;;  ())
;;;
;;;(defmethod port-set-sheet-transformation
;;;    ((port cl-gtk2-port) (sheet mirrored-sheet-mixin) transformation)
;;;  ())
...
(defmethod (setf port-motion-hints)
    (value (port cl-gtk2-port) (sheet mirrored-sheet-mixin))
  (with-gtk ()
...
(defmethod make-graft
    ((port cl-gtk2-port) &key (orientation :default) (units :device))
  (make-instance 'cl-gtk2-graft
                 :port port :mirror (gensym)
                 :orientation orientation :units units))

(defmethod text-style-mapping
    ((port cl-gtk2-port) text-style &optional character-set)
  (error "text-style-mapping called, what now?"))

(defmethod (setf text-style-mapping)
    (font-name (port cl-gtk2-port)
     (text-style text-style) &optional character-set)
  (error "(setf text-style-mapping) called, what now?"))

(defmethod port-character-width ((port cl-gtk2-port) text-style char)
  (error "port-character-width called, what now?"))

(defmethod port-string-width
    ((port cl-gtk2-port) text-style string &key (start 0) end)
  (error "port-string-width called, what now?"))

(defmethod port-mirror-width ((port cl-gtk2-port) sheet)
...
(defmethod port-mirror-height ((port cl-gtk2-port) sheet)
  (cffi:with-foreign-object (r 'gtkrequisition)
    (gtk_widget_size_request
     (mirror-widget (climi::port-lookup-mirror port sheet))
     r)
...
(defmethod port-mirror-width ((port cl-gtk2-port) (sheet cl-gtk2-graft))
  (graft-width sheet))

(defmethod port-mirror-height ((port cl-gtk2-port) (sheet cl-gtk2-graft))
  (graft-height sheet))

(defmethod graft ((port cl-gtk2-port))
...
(defmethod pointer-position ((pointer cl-gtk2-pointer))
  (%gdk-display-get-pointer))

(defmethod pointer-button-state ((pointer cl-gtk2-pointer))
  (with-gtk ()
    (gdkmodifiertype->all-buttons (nth-value 2 (%gdk-display-get-pointer)))))

(defmethod port-modifier-state ((port cl-gtk2-port))
  (with-gtk ()
...
(defmethod synthesize-pointer-motion-event ((pointer cl-gtk2-pointer))
  (with-gtk ()
...
(defmethod port-frame-keyboard-input-focus ((port cl-gtk2-port) frame)
  (with-gtk ()
...
(defmethod port-force-output ((port cl-gtk2-port))
  (with-gtk ()
    (loop
...
(defmethod port-grab-pointer ((port cl-gtk2-port) pointer sheet)
  (with-gtk ()
...
(defmethod port-ungrab-pointer ((port cl-gtk2-port) pointer sheet)
...
(defmethod distribute-event :around ((port cl-gtk2-port) event)
  (let ((grab-sheet (pointer-grab-sheet port)))
    (if grab-sheet
	(queue-event grab-sheet event)
	(call-next-method))))

(defmethod set-sheet-pointer-cursor ((port cl-gtk2-port) sheet cursor)
  ())        

(defmethod bind-selection ((port cl-gtk2-port) window &optional time)
  ())

(defmethod release-selection ((port cl-gtk2-port) &optional time)
  ())

(defmethod request-selection ((port cl-gtk2-port) requestor time)
  ())

(defmethod get-selection-from-event ((port cl-gtk2-port) event)
  ())

(defmethod send-selection ((port cl-gtk2-port) event string)
  nil)
